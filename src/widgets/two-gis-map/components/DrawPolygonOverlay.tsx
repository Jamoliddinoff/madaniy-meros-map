import type { DrawPhase } from "../hooks/useDrawPolygon";
...
/** Xarita o'ng yuqorisidagi poligon chizish jarayoni paneli (guide + Saqlash/Bekor qilish). */
  ...
  return (
      <div>
        <div className="absolute right-[50%] translate-x-[50%] top-4 z-sticky flex flex-col items-end gap-2">
          <p className="max-w-lg rounded-lg bg-neutral-0/95 px-3 py-2 text-right text-sm text-neutral-900 shadow-header backdrop-blur">
            Poligon nuqtalarini xaritada (xohlagan joyda) bosib belgilang.{" "}
            {canSave
                ? `${pointCount} ta nuqta belgilandi. Yakunlash uchun "Saqlash", Enter yoki ikki marta bosing.`
                : `Kamida ${MIN_POINTS} ta nuqta kerak (${pointCount}/${MIN_POINTS}).`}
          </p>
        </div>
